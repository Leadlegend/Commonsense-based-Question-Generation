...
class BeamSearcher(object):
    ...
    def decode(self):
        ...
        for i, eval_data in enumerate(self.data_loader):
            ...
            _, tag_seq, cs_seq, mask_seq, oov_lst = eval_data

            best_question = self.beam_search(src_seq, ext_src_seq, tag_seq, cs_seq, mask_seq)
            # discard START  token
    ...
    def beam_search(self, src_seq, ext_src_seq, tag_seq, cs_seq, mask_seq):
        enc_mask = torch.sign(src_seq)
        src_len = torch.sum(enc_mask, 1)
        prev_context = torch.zeros(1, 1, 2 * config.hidden_size)
        embed_mask = torch.ByteTensor(mask_seq.size(0), mask_seq.size(1), mask_seq.size(2), config.graph_vector_size)
        ...
        # forward encoder
        enc_outputs, enc_states = self.model.encoder(src_seq, src_len, tag_seq, cs_seq, mask_seq, embed_mask)
